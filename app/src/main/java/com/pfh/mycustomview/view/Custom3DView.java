...
public class Custom3DView extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        //拿到父控件初步给我们测量好的宽高，在xml中设置这个viewGroup的宽高均为具体数值
        int measureWidth = MeasureSpec.getSize(widthMeasureSpec);
        int measureHeight = MeasureSpec.getSize(heightMeasureSpec);
        setMeasuredDimension(measureWidth,measureHeight);

        MarginLayoutParams params = (MarginLayoutParams) this.getLayoutParams();
        //本viewGroup减去margin值后剩余的大小传给子view
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i <getChildCount() ; i++) {
            ...
            if (child.getVisibility() != GONE){
                ...
                childTop = childTop + child.getMeasuredHeight();//让子控件从上到下排列
            }
        }
    }
    ...
    protected void dispatchDraw(Canvas canvas) {
        for (int i = 0;i<getChildCount();i++){
            drawScreen(canvas,i,getDrawingTime());//绘制这个view开始的时间
        }
    }
    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {//除了down事件，其他都拦截
        switch (ev.getAction()){
        ...
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (!mScroller.isFinished()){
                    mScroller.setFinalY(mScroller.getCurrY());//设置终止时的偏移量
                    mScroller.abortAnimation();//取消动画
    ...
    private void drawScreen(Canvas canvas, int screen, long drawingTime) {
        // 得到当前子View的高度
        final int height = getHeight();//子view高度和viewGroup高度一样
